...
export default function PrestasiCreate() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 border-l-8 border-emerald-600 pl-4">Tambah Prestasi Baru</h1>
          <p className="text-gray-500 text-sm mt-1 pl-6">Catat prestasi baru siswa atau sekolah</p>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-100">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input required placeholder="Cth: Juara 1 Olimpiade Matematika 2026" className="w-full border-2 border-slate-200 focus:border-emerald-500 rounded-lg p-3 outline-none font-bold transition" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
          </div>

          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">Deskripsi (Opsional)</label>
            <div className="bg-white rounded-lg overflow-hidden border">
              <ReactQuill theme="snow" value={form.description || ''} onChange={val => setForm({ ...form, description: val })} modules={modules} className="h-32 pb-12" placeholder="Detail prestasi..." />
            </div>
          </div>
